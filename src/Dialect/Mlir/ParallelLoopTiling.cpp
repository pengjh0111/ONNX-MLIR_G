...
#define DEBUG_TYPE "parallel-loop-tiling"
...
// /// 创建并行循环tiling优化Pass
// std::unique_ptr<Pass> createParallelLoopTilingPass();

/// 主Pass实现，将并行循环tiling为优化配置
struct ParallelLoopTiling
    : public PassWrapper<ParallelLoopTiling, OperationPass<ModuleOp>> {
  ...
  // 修改构造函数接受一个布尔参数
  ParallelLoopTiling(bool noMinMaxBoundsParam = true) {
    noMinMaxBounds = noMinMaxBoundsParam;
  }
  ...
  StringRef getDescription() const final {
    return "Tile parallel loops with optimized tile sizes";
  }
  
  void runOnOperation() override;
  
  /// 使用给定配置对并行循环应用tiling变换
  std::pair<ParallelOp, ParallelOp> tileParallelLoopPlus(
      ParallelOp op, 
      const CompleteTileConfig &tileConfig, 
...
// std::unique_ptr<Pass> createParallelLoopTilingPass() {
//   return std::make_unique<ParallelLoopTilingPass>();
// }
...
void ParallelLoopTiling::runOnOperation() {
  ...
  // 创建优化器，使用默认硬件参数
  TileSizeOptimizer optimizer(context);
  
  // 查找所有可以tiling的并行循环
  SmallVector<scf::ParallelOp, 8> parallelOps;
  ...
  for (auto parallelOp : parallelOps) {
    // 优化tile大小
    CompleteTileConfig tileConfig = optimizer.optimizeTileSize(parallelOp);
    
    // 使用优化配置应用tiling
    auto tiledLoops = tileParallelLoopPlus(parallelOp, tileConfig, noMinMaxBounds);
    if (!tiledLoops.first) {
      parallelOp->emitWarning() << "Failed to apply tiling with optimized configuration";
    }
  }
}

std::pair<ParallelOp, ParallelOp> ParallelLoopTiling::tileParallelLoopPlus(
    ParallelOp op, 
    const CompleteTileConfig &tileConfig,
    bool noMinMaxBounds) {
  ...
  // 检查配置是否有效
  if (tileConfig.perDimConfig.size() != op.getNumLoops()) {
    return {nullptr, nullptr};
  }
  
  // 将tile大小生成为Value对象
  SmallVector<Value, 4> tileSizeConstants;
  tileSizeConstants.reserve(op.getUpperBound().size());
  
  for (size_t i = 0, end = op.getUpperBound().size(); i != end; ++i) {
    if (i < tileConfig.perDimConfig.size()) {
      tileSizeConstants.push_back(
          b.create<arith::ConstantIndexOp>(loc, tileConfig.perDimConfig[i].tileSize));
    } else {
      // 对于剩余维度，使用默认值1
      tileSizeConstants.push_back(b.create<arith::ConstantIndexOp>(loc, 1));
    }
  }
  
  // 创建外层循环，步长为原始步长乘以tile大小
  SmallVector<Value, 4> newSteps;
  newSteps.reserve(op.getStep().size());
  
  for (auto step : llvm::zip(op.getStep(), tileSizeConstants)) {
    newSteps.push_back(b.create<arith::MulIOp>(loc, std::get<0>(step), std::get<1>(step)));
  }
  
  auto outerLoop = b.create<ParallelOp>(
      loc, op.getLowerBound(), op.getUpperBound(), newSteps);
  
  b.setInsertionPointToStart(outerLoop.getBody());
  
  // 计算min(size, dim - offset)以避免越界访问
  auto minMap = AffineMap::get(
      /*dimCount=*/3, /*symbolCount=*/0,
      {getAffineDimExpr(/*position=*/0, b.getContext()),
       getAffineDimExpr(/*position=*/1, b.getContext()) -
  ...
  for (auto [lowerBound, upperBound, newStep, iv, step, tileSizeConstant] :
    ...
    // 如果循环边界和步长是常量，并且迭代次数是tile大小的整数倍，
    // 则为内层循环使用静态边界
    if (lowerBoundConstant && upperBoundConstant && stepConstant) {
      auto numIterations = llvm::divideCeil(upperBoundConstant.value() -
                                              lowerBoundConstant.value(),
    ...
    // 否则，我们为外层循环的每次迭代动态计算边界
    newBounds.push_back(
        b.create<affine::AffineMinOp>(loc, b.getIndexType(), minMap,
                                    ValueRange{newStep, upperBound, iv}));
  }
  
  // 创建内层循环，下界为0
  auto innerLoop = b.create<ParallelOp>(
      loc, SmallVector<Value, 4>(newBounds.size(), zero), newBounds, op.getStep());
  
  if (noMinMaxBounds && needInboundCheck) {
    b.setInsertionPointToStart(innerLoop.getBody());
    // 插入边界检查
    Value inbound =
        b.create<arith::ConstantIntOp>(loc, 1, b.getIntegerType(1));
    ...
    for (auto [outerUpperBound, outerIV, innerIV, innerStep] :
      ...
      //             (%inner_iv * %inner_step + %outer_iv < %outer_upper_bound)
      Value index = b.create<arith::AddIOp>(
          loc, b.create<arith::MulIOp>(loc, innerIV, innerStep), outerIV);
      Value dimInbound = b.create<arith::CmpIOp>(
          loc, arith::CmpIPredicate::ult, index, outerUpperBound);
      inbound = b.create<arith::AndIOp>(loc, inbound, dimInbound);
    }
    
    auto ifInbound = b.create<scf::IfOp>(loc,
                                     /*resultTypes*/ llvm::ArrayRef<Type>{}, inbound,
                                     /*hasElseRegion*/ false);
    ...
    for (const auto &ivs : llvm::enumerate(llvm::zip(
             innerLoop.getInductionVars(), outerLoop.getInductionVars()))) {
      auto newIndex = b.create<arith::AddIOp>(
          loc, std::get<0>(ivs.value()), std::get<1>(ivs.value()));
      thenBlock.getArgument(ivs.index())
          .replaceAllUsesExcept(newIndex, newIndex);
    }
    ...
    for (auto ivs : llvm::zip(innerLoop.getInductionVars(),
                            outerLoop.getInductionVars())) {
      Value innerIndex = std::get<0>(ivs);
      auto newIndex = b.create<arith::AddIOp>(loc, std::get<0>(ivs),
                                            std::get<1>(ivs));
      innerIndex.replaceAllUsesExcept(newIndex, newIndex);
    }
  }
